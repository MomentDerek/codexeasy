...
- Use npm (package-lock is present) for scripts and dependency changes.

## Testing
- For substantive code changes, run the full suite with `npm run check:all` before finishing work.
- When working on a narrow area, run the most relevant subset (e.g., `npm run lint`, `npm run typecheck`, `npm run test:run`, `npm run rust:fmt:check`, `npm run rust:clippy`, `npm run rust:test`) and note what was executed.
